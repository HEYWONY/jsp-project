...
<head>
    <title>FAQ</title>
    <link rel="stylesheet" href="smr/css/faq.css">
</head>
<body>
...
<div class="wrap">
    ...
    <div class="wrap_right">
        <div class="notice_write">
            ...
            <c:choose>
                <c:when test="${uid==100}">
                        <a href="faq_write.do" class="write_btn">글쓰기</a>
                </c:when>
            </c:choose>
        </div>

        <form method="get" action="faqlist.do" class="search">
            ...
            <c:choose>
                ...
                <c:otherwise>
                    <c:forEach var="item" items="${list}">
                        <li>
                            <span><a href="faq_detail.do?bno=${item.f_no}"><c:out value="${item.title}"/></a></span>
        ...
        <div class="pagenum">
            <c:if test="${startpage>1}">
                <a href="faqlist.do?bno=${startpage-1}&search_txt=${search_txt}">이전</a>
            </c:if>
            <c:forEach var="i" begin="${startpage}" end="${endpage}" step="1">
                ...
                <c:if test="${currpage!=i}">
                    <a href="faqlist.do?curr=${i}&search_txt=${search_txt}">${i}</a>
                </c:if>
            </c:forEach>
            <c:if test="${endpage < total_page}">
                <a href="faqlist.do?curr=${endpage+1}&search_txt=${search_txt}">다음</a>
            </c:if>
        </div>
    </div>
</div>
